...
if x0 >= 0 and y0 >=0 and x1 < width and y1 < height:  # 请求瓦片完全在影像内部
    ...
    if ((x1-x0)/tile_size) > 20 or ((y1-y0)/tile_size) > 20: # 请求层级太小，内存溢出处理
        ...
        band1_part_data = ds.GetRasterBand(1).ReadAsArray(0, 0, (width - 1), (height - 1), int(width/times), int(height/times))
        band2_part_data = ds.GetRasterBand(2).ReadAsArray(0, 0, (width - 1), (height - 1), int(width/times), int(height/times))
        band3_part_data = ds.GetRasterBand(3).ReadAsArray(0, 0, (width - 1), (height - 1), int(width/times), int(height/times))
        ...
        band1_data[int((0-y0)/times):int(int(height/times) + int((0-y0)/times)),int((0-x0)/times):int(int(width/times) + int((0-x0)/times))] = band1_part_data
        band2_data[int((0-y0)/times):int(int(height/times) + int((0-y0)/times)),int((0-x0)/times):int(int(width/times) + int((0-x0)/times))] = band2_part_data
        band3_data[int((0-y0)/times):int(int(height/times) + int((0-y0)/times)),int((0-x0)/times):int(int(width/times) + int((0-x0)/times))] = band3_part_data

    else:
        band1_part_data = ds.GetRasterBand(1).ReadAsArray(0, 0, (width - 1), (height - 1))
        band2_part_data = ds.GetRasterBand(2).ReadAsArray(0, 0, (width - 1), (height - 1))
        band3_part_data = ds.GetRasterBand(3).ReadAsArray(0, 0, (width - 1), (height - 1))
