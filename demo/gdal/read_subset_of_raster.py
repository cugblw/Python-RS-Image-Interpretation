...
tif_path = r"C:\Users\cugbl\Desktop\mlxy_C1_jlp.tif"
...
if x0 >= 0 and y0 >= 0 and x1 < width and y1 < height:  # 请求瓦片完全在影像内部
    ...
    if ((x1-x0)/tile_size) > 20 or ((y1-y0)/tile_size) > 20: # 请求层级太小，内存溢出处理
    ...
    if ((x1-x0)/tile_size) > 20 or ((y1-y0)/tile_size) > 20: # 请求层级太小，内存溢出处理
        ...
        if x0 < 0 and 0 <= x1 < width and y0 < 0  and y1 < height: # situation 1'
            ...
            band1_part_data =  ds.GetRasterBand(1).ReadAsArray(0, y0, x1, (height-x0), int(x1/times), int((height-y0)/times))
            band2_part_data =  ds.GetRasterBand(2).ReadAsArray(0, y0, x1, (height-x0), int(x1/times), int((height-y0)/times))
            band3_part_data =  ds.GetRasterBand(3).ReadAsArray(0, y0, x1, (height-x0), int(x1/times), int((height-y0)/times))

            band1_data = np.zeros((int((y1-y0)/times),int((x1-x0)/times)))
            band2_data = np.zeros((int((y1-y0)/times),int((x1-x0)/times)))
            band3_data = np.zeros((int((y1-y0)/times),int((x1-x0)/times)))

            band1_data[int((0-x0)/times):(int(x1/times)-int(x0/times)),0:int((0-y0)/times)] = band1_part_data
            band2_data[int((0-x0)/times):(int(x1/times)-int(x0/times)),0:int((0-y0)/times)] = band2_part_data
            band3_data[int((0-x0)/times):(int(x1/times)-int(x0/times)),0:int((0-y0)/times)] = band3_part_data
        
        elif 0 <= x0 < width and x1 >= width and y0 < 0 and 0 <= y1 < height: # situation 3'
            band1_part_data =  ds.GetRasterBand(1).ReadAsArray(x0, 0, (width-x0), y1, int((width-x0)), int(y1))
            band2_part_data =  ds.GetRasterBand(2).ReadAsArray(x0, 0, (width-x0), y1, int((width-x0)), int(y1))
            band3_part_data =  ds.GetRasterBand(3).ReadAsArray(x0, 0, (width-x0), y1, int((width-x0)), int(y1))
            ...
            band1_part_data =  ds.GetRasterBand(1).ReadAsArray(x0, y0, (width-x0), (y1-y0), int((width-x0)/times), int((y1-y0)/times))
            band2_part_data =  ds.GetRasterBand(2).ReadAsArray(x0, y0, (width-x0), (y1-y0), int((width-x0)/times), int((y1-y0)/times))
            band3_part_data =  ds.GetRasterBand(3).ReadAsArray(x0, y0, (width-x0), (y1-y0), int((width-x0)/times), int((y1-y0)/times))
        ...
        if x0 < 0 and 0 <= x1 < width and y0 < 0  and y1 < height: # situation 1
            ...
            band1_data[(0-x0):(x1-x0),0:(0-y0)] = band1_part_data
            band2_data[(0-x0):(x1-x0),0:(0-y0)] = band2_part_data
            band3_data[(0-x0):(x1-x0),0:(0-y0)] = band3_part_data
