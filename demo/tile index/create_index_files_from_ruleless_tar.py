...
def create_index_file_from_ruleless_tar(start_zoom, end_zoom, tar_list_dict, index_dir):
    for tile_id in tar_list_dict.keys():
        zoom = int(tile_id.split('_')[0])
        x = int(tile_id.split('_')[1])
        y = int(tile_id.split('_')[2])
        if zoom != start_zoom:
        ...
        tar_list = tar_list_dict[tile_id]
        members_list = []
        for tar_file in tar_list:
            with tarfile.open(tar_file,'r') as tar:
                members = tar.getnames()
...
if __name__ == '__main__':
    start_zoom = 10
    end_zoom = 18
    tar_dir = r"C:\Users\Administrator\Desktop\tar_test"
    tile_dir = r"C:\Users\Administrator\Desktop\tar_test"
    index_dir = r"C:\Users\Administrator\Desktop\index\test"
    tar_list_dict = ci.get_tar_list_dict_from_ruleless_tar(tar_dir)
    ...
    # 从tar文件中创建索引文件
    create_index_file_from_ruleless_tar(start_zoom, end_zoom, tar_list_dict, index_dir)
